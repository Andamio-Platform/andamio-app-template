...
exports.createMockSubmissionContext = createMockSubmissionContext;
exports.createMockConfirmationContext = createMockConfirmationContext;
exports.testSideEffect = testSideEffect;
/**
 ...
 * ```
 */
function resolvePathParams(endpoint, pathParams, context) {
    if (!pathParams)
        return endpoint;
    ...
    for (const [paramName, contextPath] of Object.entries(pathParams)) {
        ...
        if (value === undefined) {
            throw new Error(`Path parameter "${paramName}" could not be resolved from context path "${contextPath}"`);
        }
 ...
 * ```
 */
function constructRequestBody(bodyDef, context) {
    if (!bodyDef)
        return undefined;
    const body = {};
    ...
    console.log("[constructRequestBody] context.buildInputs:", context.buildInputs);
 ...
 */
function validateSideEffect(sideEffect, context) {
    const errors = [];
    ...
    if (["POST", "PATCH", "PUT"].includes(sideEffect.method) &&
        !sideEffect.body) {
 ...
 */
function createMockSubmissionContext(overrides) {
    return {
        txHash: "abc123def456",
 ...
 */
function createMockConfirmationContext(overrides) {
    return {
        txHash: "abc123def456",
        signedCbor: "84a30081825820...",
        buildInputs: {
 ...
 */
function testSideEffect(sideEffect, context) {
    const errors = validateSideEffect(sideEffect, context);
    if (errors.length > 0) {
        return { valid: false, errors };
    }
    try {
        const resolvedEndpoint = resolvePathParams(sideEffect.endpoint, sideEffect.pathParams, context);
