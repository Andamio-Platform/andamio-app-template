 ...
 * ```
 */
export function decodeTransactionCbor(cborHex: string): DecodedTransaction {
  ...
  const referenceInputs: DecodedInput[] = [];
  ...
  let metadata: Map<bigint, unknown> | undefined;
 ...
 * ```
 */
export function extractMints(cborHex: string): DecodedMint[] {
 ...
 * ```
 */
export function extractMintsByPolicy(cborHex: string, policyId: string): DecodedMint[] {
 ...
 * ```
 */
export function extractAssetNames(cborHex: string, policyId?: string): string[] {
 ...
 */
export function extractTxId(cborHex: string): string {
  const tx = deserializeTx(cborHex);
