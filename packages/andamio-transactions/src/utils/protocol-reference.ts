 ...
 * ```
 */
export function createProtocolSpec(
 ...
 */
export function getProtocolCost(id: string): import("../types").TransactionCost {
  const cost = PROTOCOL_COSTS[id as keyof typeof PROTOCOL_COSTS];
  if (!cost) {
    return { txFee: 200000 }; // Default estimate
  }

  // Convert readonly arrays to mutable arrays for compatibility
  const result: import("../types").TransactionCost = {
    txFee: cost.txFee,
  };
